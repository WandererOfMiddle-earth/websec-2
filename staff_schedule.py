...
def parser(url):
    ...
    title = re.findall(r"<h2 class=\"h2-text info-block__title\">(.*?)<\/h2>", raw)
    ...
    if (len(raw) != 0):
        for element in raw:
            if element[0] == "" and element[3] == "":
                ...
                lesson["group"] = lesson["group"][:-1]
                data["cells"].append(lesson)

    with open("jsons/staff_schedule.json", "w", encoding="utf-8") as file:
        json.dump(data, file, indent=4, ensure_ascii=False)

parser("https://ssau.ru/rasp?staffId=62061001&selectedWeek=26&selectedWeekday=1")